...
export function EmptyState({ icon: Icon, title, description, action, className }: EmptyStateProps) {
  return (
    ...
    >
      <div className="mb-4 flex h-12 w-12 items-center justify-center rounded-full bg-sevs-primary-muted">
        <Icon className="h-6 w-6 text-sevs-primary" aria-hidden />
      </div>
      ...
      {action && (
        <Button onClick={action.onClick} className="mt-4 bg-sevs-primary text-white hover:bg-sevs-primary-light">
          {action.label}
        </Button>
      )}
    </div>
